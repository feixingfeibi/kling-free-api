...
- URL signing is not implemented yet. Raw HTTP replay may fail even with a valid Cookie, because the web client signs requests before dispatch.
...
- the final `__NS_hxfalcon` generation step still needs either signer reconstruction or browser-context forwarding

## Practical next step

Either add the web signing layer, or switch to a browser-context forwarding approach first.
